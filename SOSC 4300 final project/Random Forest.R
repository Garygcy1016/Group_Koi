...
nrow(data)
data <- data[!(is.na(data$monetary_gain) | is.na(data$status_type)),]
nrow(data)

set.seed(100)
data.imputed <- rfImpute(monetary_gain ~ ., data = data, iter=100)
str(data.imputed)
...
ggsave("/Users/choyuching/Downloads/0136_deltedNAin monetary_gain.pdf")
...
for(i in 1:10) {
  temp.model <- randomForest(monetary_gain ~ ., data=data.imputed, mtry=i, ntree=1000)
...
ggsave("/Users/choyuching/Downloads/22_12_random_forest_mds_plot_monetary gain.pdf")